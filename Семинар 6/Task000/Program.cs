...
{
    int[] numbers = new int[length]; //создание пустого массива
    Random random = new Random(); // создание генератора случйных чисел
    for (int i=0; i < numbers.Length; i++) // заполнение массива случайными числами
    {
        numbers[i] = random.Next(minValue, maxValue + 1); //заполнение текущекго элемеента массива случайным числом в диапазоне [minValue, maxValue]
    }
    return numbers;
}
void printArray(int[] array)
{
    ...
    {
        Console.Write($", {array[i]}"); // вывод теущего элемента массива
    }
    Console.WriteLine();                    // перевод строки (чтобы отделить массивы)
}
...
int [] newArray = generateArray(7, 0, 10);
printArray(newArray);
